...
export default function Header() {
  ...
  return (
    <header className="fixed top-6 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-3rem)] max-w-5xl">
      <div 
        className={`pro-panel px-8 transition-all duration-300 hover:translate-y-0 ${
          isOpen 
            ? 'rounded-[2rem] py-6 flex flex-col gap-6 h-auto' 
            : 'rounded-full h-16 flex items-center justify-between'
        }`}
      >
        {/* Top header row containing Logo, Desktop Menu, Auth Button and Mobile Toggle */}
        <div className="w-full flex items-center justify-between">
          {/* Logo (Left) */}
          <Link href="/" className="text-xl font-bold tracking-tighter hover:opacity-80 transition-opacity font-display">
            VIBE<span className="text-neutral-500 italic">CLOSET</span>
          </Link>

          {/* Desktop Navigation Links (Center) - hidden on mobile */}
          <nav className="hidden md:flex items-center gap-8 text-[11px] font-bold tracking-[0.2em] uppercase text-neutral-400">
        ...
        {/* Mobile Navigation Drawer (Visible only when open and on mobile screen sizes) */}
        {isOpen && (
